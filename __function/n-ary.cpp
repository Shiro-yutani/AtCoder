...
string n_ary(string str, int n, int m){
    unsigned long tmp = 0;
    string ret;
//    cout << __func__ << ", " << __LINE__ << ": str.length()= " << str.length() << endl;
    for(int i=0; i<str.length(); i++){
//        cout << __func__ << ", " << __LINE__ << ": i= " << i << ", str[str.length()-1-i]= " << str[str.length()-1-i] << endl;
        tmp += (unsigned long) ntodec(str[str.length()-1-i]) * pow_ll(n, i);
    }
//    cout << __func__ << ", " << __LINE__ << ": converting to decimal finished. tmp= " << tmp << endl;
    if(tmp==0) return "0";
    while(tmp!=0){
//        cout << __func__ << ", " << __LINE__ << ": ret= " << ret << endl;
        ret = decton(tmp%m) + ret;
        //ret.insert(decton(tmp%m),0);
        tmp/=m;
//        cout << __func__ << ", " << __LINE__ << ": ret= " << ret << endl;
    }
    return ret;
...
/*  関数名          ntodec(const char c)
    説明            char で 文字{0,1,2,...,9,A,B,...,Z} を受け取り、int で {0,1,2,...9,10,11,...,35}に直して返す。それ以外の文字が来た場合は-1を返す。
    使用ライブラリ  なし
*/
int ntodec(const char c){
    switch(c){
...
/*  関数名          decton(const int n)
    説明            int で 数値 0〜35 を受け取り、文字{0,1,2,...,9,A,B,...,Z}に直して char で返す。それ以外の数字が来た場合は \0 を返す。
    使用ライブラリ  なし
*/
char decton(const int n){
    switch(n){
